...
const BLOCK_TIME = { mainnet: 12, base: 2, arbitrum: 0.25, optimism: 2 };
 ...
 */
function readTailRows(file, needFrom) {
    ...
    try {
        ...
        while (pos > 0) {
            ...
            if (fromBlock === null && rows.length)
                fromBlock = needFrom(Math.max(...rows.map((r) => r.b)));
            if (fromBlock !== null && rows.length && Math.min(...rows.map((r) => r.b)) <= fromBlock)
                break;
        }
...
function resampled(sec) {
    const bucket = Math.max(1, Math.floor(sec / bt));
    const m = new Map();
    for (const r of w)
        m.set(Math.floor(r.b / bucket), price(r));
    const ks = [...m.keys()].sort((a, b) => a - b);
    let s = 0;
    let n = 0;
    for (let i = 1; i < ks.length; i++) {
        if (ks[i] - ks[i - 1] !== 1)
            continue; // luki pomijamy, nie sklejamy
...
for (const r of w) {
    const p = price(r);
    if (prev)
        sumR2 += Math.log(p / prev) ** 2;
    prev = p;
}
const netMove = Math.abs(Math.log(price(w[w.length - 1]) / price(w[0])));
const trendRatio = sumR2 > 0 ? netMove / Math.sqrt(sumR2) : null;
const width = (sigma, k) => Math.min(Math.max(k * sigma * Math.sqrt(7), 0.01), 0.6);
const line = (nazwa, v) => v === null
...
for (const [nm, sec] of [['1 min', 60], ['5 min', 300], ['15 min', 900], ['1 h', 3600]])
...
if (ref) {
    ...
    if (noiseDecay !== null)
        console.log(`spadek σ z próbki 5min→1h: ${(noiseDecay * 100).toFixed(0)}%  ` +
...
if (trendRatio !== null) {
    ...
    console.log('WNIOSEK: dla ustawiania zakresu liczy się WIELKOŚĆ RUCHU w horyzoncie, nie „chop".\n' +
